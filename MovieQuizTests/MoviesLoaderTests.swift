    ...
    func testSuccessLoading() throws {
        ...
        loader.loadMovies { result in
            ...
            case .success(let movies):
                XCTAssertEqual(movies.items.count, 2) // сравниваем два фильма, потому что в тестовых данных 2
                expectation.fulfill() // Когда результат функции будет получен, мы скажем, что ожидание было выполнено
            case .failure(_):
                XCTFail("Unexpected failure") // проваливаем тест если пришла ошибка
            }
        }
...
struct StubNetworkClient: NetworkRouting {
    ...
    let emulateError: Bool // заглушка ошибки сети или успешного ответа
